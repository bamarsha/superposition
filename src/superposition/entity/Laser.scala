...
object Laser {
  ...
  def apply(
    ...
    entity.add(new Beam(multiverse, gate, direction, control))
    entity.add(new Outline(control map (_.any), cell map (_.toDouble), Vector2(2, 2)))
    entity.add(new Renderable(1.pure[QExpr], frame))
    entity.add(new SpriteView(frame, baseTexture, scale = Vector2(2.0, 2.0).pure[QExpr]))
