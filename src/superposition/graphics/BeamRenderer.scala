    ...
    for {
      ...
      for (i <- Seq.range(0, numBeams)) if (beamSeq.get(i)) {
        ...
        beam.direction match {
          case Left | Right => shapeRenderer.rect(source.x + 0.5f, source.y + beamOffset(i), target.x - source.x, beamWidth)
          case Up | Down => shapeRenderer.rect(source.x + beamOffset(i), source.y + 0.5f, beamWidth, target.y - source.y)
        }
      }
      shapeRenderer.end()
