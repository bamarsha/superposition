...
import superposition.math.QExpr.QExprMonad
import superposition.math.{BitSeq, QExpr, StateId, Vector2}
...
private final class BuiltIns(multiverse: Multiverse, map: TiledMap) {
  ...
  val and: QExpr[Iterable[Boolean] => Boolean] = ((_: Iterable[Boolean]) forall identity).pure

  /** Returns the bit at the index in the bit sequence. */
  val bitAt: QExpr[((BitSeq, Int)) => Boolean] = ((_: BitSeq) (_: Int)).tupled.pure

  /** Returns the cell in game coordinates corresponding to the (x, y) position in tile map coordinates. */
  val cell: QExpr[((Int, Int)) => Vector2[Int]] = QExprMonad.pure { case (x, y) => Vector2(x, height - y - 1) }
  ...
  def indices[A]: QExpr[((Seq[A], Seq[Int])) => Seq[A]] =
    QExprMonad.pure { case (items, indices) => indices map (items(_)) }

  /** Converts a bit sequence to an integer. */
  val int: QExpr[BitSeq => Int] = ((_: BitSeq).toInt).pure

  /** Returns true if any bit is true. */
  val or: QExpr[Iterable[Boolean] => Boolean] = ((_: Iterable[Boolean]) exists identity).pure

  /** Returns the primary qubits for the entity with the ID. */
  val qubits: QExpr[Int => Seq[StateId[Boolean]]] =
    (multiverse.entityById(_: Int).get.getComponent(classOf[PrimaryBit]).bits).pure
  ...
  val qucell: QExpr[Int => StateId[Vector2[Int]]] =
    (multiverse.entityById(_: Int).get.getComponent(classOf[QuantumPosition]).cell).pure

  /** Returns the value of the qudit. */
  def value[A]: QExpr[StateId[A] => A] = QExpr.prepare(_.value)

  /** Converts a 2-tuple into a 2-vector. */
  val vec2: QExpr[((Int, Int)) => Vector2[Int]] = (Vector2(_: Int, _: Int)).tupled.pure
}
